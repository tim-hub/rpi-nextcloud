...
RUN wget -q -O - https://download.nextcloud.com/server/releases/nextcloud-11.0.3.tar.bz2 | tar jx -C /var/www/;chown -R www-data:www-data /var/www/nextcloud
...
ADD resources/001-nextcloud.conf /etc/apache2/sites-available/
...
RUN a2ensite 001-nextcloud.conf
